...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
                xmlns:prefix="https://hocvienboardgame.vn/" 
                xmlns:exsl="http://exslt.org/common"
                version="1.0">
    <xsl:output method="xml" indent="yes"/>

    <xsl:import href="hocvienboardgame-category.xsl"/>
    ...
    <xsl:template name="CrawlTheLink">
        ...
        <xsl:if test="$page//div[@class='nav-links']/a[contains(@class,'next')]">
            <xsl:variable name="nextLink" select="$page//div[@class='nav-links']/a[contains(@class,'next')]/@href"/>
            <xsl:call-template name="CrawlTheLink">
